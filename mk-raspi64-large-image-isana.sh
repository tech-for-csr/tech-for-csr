...
NOW=$(date +"%d.%m.%Y-%H%M")
IN_IMAGE_DIR=/home/isana/omni-rom/out/target/product/rpi4/
IN_BOOT_FILES=/home/isana/omni-rom/vendor/brcm/rpi4/proprietary/boot
...
sudo mkfs.ext4 /dev/mapper/loop13p2 -L system
sudo mkfs.ext4 /dev/mapper/loop13p3 -L vendor
sudo mkfs.ext4 /dev/mapper/loop13p4 -L userdata
echo "enable project quota"
sudo tune2fs -O project,quota /dev/mapper/loop13p4

echo "write system.img"
sudo dd if="$IN_IMAGE_DIR/system.img" of=/dev/mapper/loop13p2 bs=1M
echo "write vendor.img"
sudo dd if="$IN_IMAGE_DIR/vendor.img" of=/dev/mapper/loop13p3 bs=1M

echo "write boot patition"
sudo mkdir /mnt/tmp
sudo mount /dev/mapper/loop13p1 /mnt/tmp
